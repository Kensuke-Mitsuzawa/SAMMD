...
path_work_root = '/content/drive/MyDrive/developments/colab_storage/phd/exps/SAMMD'
path_dir_trained_model = Path(path_work_root).joinpath('Res18_model')
...
trainset = torchvision.datasets.CIFAR10(
    root=path_work_root, train=True, download=True, transform=transform_test)  # training dataset
# Generate batch by batch for batch training, and the order of composition batch is shuffled
trainloader = torch.utils.data.DataLoader(
    trainset, batch_size=128, shuffle=False, num_workers=0)
testset = torchvision.datasets.CIFAR10(
    root=path_work_root, train=False, download=True, transform=transform_test)
...
if __name__ == "__main__":
    ...
    path_dir_trained_model.mkdir(parents=True, exist_ok=True)
    path_acc_text = path_dir_trained_model.joinpath('acc.txt')
    path_log_text = path_dir_trained_model.joinpath('log.txt')

    with open(path_acc_text, "w") as f:
        with open(path_log_text, "w")as f2:
